...
import { GAME_TOP_MIDDLE, GAME_TOP_UPPER, SVG_SCALE } from '@/game/constants'
...
export default defineComponent({
  ...
  setup () {
    ...
    const timeCountTranslateX = computed(() => {
      return (GAME_TOP_MIDDLE.widthLeft + store.getters.getDisplayWidth * store.state.squareSize - 41 - 3) * SVG_SCALE
    })
    // 当前计数器显示的剩余雷数
    ...
    // 遮罩的宽度
    const maskWidth = computed(() => (store.getters.getDisplayWidth * store.state.squareSize + 2) * SVG_SCALE)
    // 遮罩的高度
