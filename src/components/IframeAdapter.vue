...
export default defineComponent({
  ...
  setup () {
    ...
    const setShareLink = (share?: Share) => {
      ...
      if (!share?.uri) return
      // 页面路径
      const path = `${parent.location.origin}${share.pathname ? share.pathname : '/'}`
      try {
    ...
    // 暴露接口给父窗口
    parent.flop = parent.flop || {}
    // 播放录像文件
    parent.flop.playVideo = (uri: string, { anonymous, background, share, listener }: { anonymous?: boolean, background?: string, share?: Share, listener?: () => void }) => {
      // 设置页面退出状态以显示播放页面
    ...
    // 解析录像文件
    parent.flop.parseFiles = (fileList: FileList | undefined | null, onload: (video: BaseParser) => void, onerror?: (info: string) => void) => {
      onerror = onerror || (() => ({}))
